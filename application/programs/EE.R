...
##  estimating equations for recurrent cure model ##
   ...
   ee_beta.fn=function(x){
      ...
      for(i in 1:n){
         ...
         for(j in 1:Ki[i]){
            cnt=min(which(nX[i,j]==tt))
            #tmpj[j,]=Z[i,]*(delta[i]*1*(nX[i,j]<=tt[length(tt)])-log(L.fn(Lambda.fn(ht[cnt]+bz[i])-Wij[i,,j]%*%gamma))+log(L.fn(-Wij[i,,j]%*%gamma))) old vision as in windows
            tmpj[j,]=c(c(Z[i,])*(delta[i]*1*(nX[i,j]<=tt[length(tt)])-log(L.fn(Lambda.fn(ht[cnt]+bz[i])-Wij[i,,j]%*%gamma))+log(L.fn(-Wij[i,,j]%*%gamma))))
         }
   ...
   ee_gamma.fn=function(x){
      ...
      for(i in 1:n){
         ...
         for(j in 1:Ki[i]){                     
            cnt=min(which(nX[i,j]==tt))
            tmpj[j,]=Wij[i,,j]*( delta[i]+(1-delta[i])*L.bn(Lambda.fn(ht[cnt]+bz[i])-Wij[i,,j]%*%gamma)-(L.fn(Wij[i,,j]%*%x)))           
         }
         tmpi[i,]=colSums(tmpj)/Ki[i]
      }
      return(colSums(tmpi))
